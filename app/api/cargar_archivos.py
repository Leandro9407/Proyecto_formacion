...
async def upload_excel(
    ...
    df = pd.read_excel(
        ...
        engine="openpyxl",
        skiprows=4,
        usecols=["IDENTIFICADOR_FICHA", "CODIGO_CENTRO", "CODIGO_PROGRAMA", "VERSION_PROGRAMA", "NOMBRE_PROGRAMA_FORMACION", "ESTADO_CURSO", "NIVEL_FORMACION",
                 "NOMBRE_JORNADA", "FECHA_INICIO_FICHA", "FECHA_TERMINACION_FICHA", "ETAPA_FICHA", "MODALIDAD_FORMACION", "NOMBRE_RESPONSABLE", "NOMBRE_EMPRESA",
                 "NOMBRE_MUNICIPIO_CURSO", "NOMBRE_PROGRAMA_ESPECIAL", "NOMBRE_PROGRAMA_FORMACION"],  # Nombres reales
        dtype=str
    )
    ...
    #Si quiere que funcione en todos los centros del pais, crear codigo para llenar regionales centros y eliminar la siguiente linea
    ...
    required_fields = [
        "cod_ficha", "cod_centro", "cod_programa", "la_version", "nombre", "fecha_inicio", "fecha_fin", "etapa", "responsable", "nombre_municipio"
    ]
    ...
    # Eliminar la columa nombre del df
    df = df.drop('nombre', axis=1)  # Eliminar columna 'nombre' ya que está en df_programas
    print(df.head())
